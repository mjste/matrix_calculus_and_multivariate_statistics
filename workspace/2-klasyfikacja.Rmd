...
W trakcie zajęć wykorzystywany będzie zbiór danych `Smarket` z pakietu
`ISLR`. Opisuje on zachowanie indeksu giełdowego S&P 500 przez kolejne
dni od roku 2001 do roku 2005. Zmienna `Direction` jest czynnikiem o 2
poziomach podsumowującym kierunek zmiany indeksu w danym dniu, tzn.
`Direction == "Up"` wtedy i tylko wtedy, gdy `Today >= 0`. [**Sprawdź**]
...
`Direction` na podstawie zmiennych `Lag1`, ..., `Lag5` i `Volume`.
...
[**2. Jaka jest zatem proporcja błędów dla takiego zbioru testowego?**]

[**3. Zaimplementuj losowy podział zbioru na treningowy i testowy w
stosunku ilościowym 70:30. Wytrenuj model i porównaj wyniki.**]

```{r train-test split task}
# Miejsce na Twój kod.
```

Próbujemy uzyskać bardziej efektywny model eliminując najmniej istotne
predyktory --- wszystkie mają słabe lub bardzo słabe $p$-wartości, ale
najlepsze dwa to `Lag1` i `Lag2`.
...
[**4. Jak w tym przypadku wygląda proporcja błędów? Jak wygląda jej
porównanie z proporcją błędów naiwnej strategii przewidującej wzrost
zawsze?**]
...
[**5. Jaką strategię inwestowania można zasugerować na podstawie
ostatniej tabeli pomyłek?**]
...
przez `Direction` względem `Lag1` i `Lag2`.
...
[**6. Jak wygląda porównanie wyniku klasyfikacji LDA z regresją
logistyczną?**]
...
[**7. Jaka jest w tym przypadku proporcja błędów?**]

[**8. Wykorzystując zbiór danych Hitters zawierający dane zawodników ligi baseballowej wykonaj klasyfikację dywizji, w której gra dany gracz. Wykorzystaj model regresji logistycznej i jeden z modeli LDA lub QDA. Który sprawdził się lepiej? Spróbuj wyznaczyć najlepiej sprawdzającą się kombinację predyktorów.**]

```{r hitters dataset prep}
summary(Hitters)
```

```{r hitters dataset task}
# Miejsce na Twój kod.
```
...
[**9. Jak wygląda proporcja błędów dla** $k = 3$? A dla większych $k$?]

[10. Klasyfikator kNN traci skuteczność w przypadku problemów o wysokiej
wymiarowości. Co jest tego powodem?... Załóżmy problem o n wymiarach,
gdzie wszystkie cechy X_1, ..., X_n są wylosowane z rozkładu
jednostajnego na przedziale (0,1). Do klasyfikacji wybranego punktu
p\*\* $$x_1,...,x_n$$ chcemy wykorzystać zmienne znajdujące się w 1/10
przedziału wokół punktu p dla każdej z cech. W zależności od wymiaru,
jaka część punktów zostanie wzięta od uwagę? Jak ustawić wielkość
przedziału, by zawsze brać ich 10%?]
